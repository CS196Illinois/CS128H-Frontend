...
export class GradesComponent implements OnInit {
  ...
  displayGrades(data) {
    ...
    var FinalProject = 0;
    this.FinalGrade =
      grades.final_grade == 1 ? "Satisfactory" : "Unsatisfactory";
    for (var i = 0; i < grades.grades.length; ++i) {
      if (grades.grades[i]["assignment_name"].substring(0, 2) == "MP") {
        ...
        MPGradeCount++;
        this.mp_grades.push({
          assignmentName: grades.grades[i]["assignment_name"],
          grade: grades.grades[i]["grades"],
        });
    ...
    this.final_grade =
      Math.round(
        ((MPGradeTotal / MPGradeCount) * 0.5 +
          (HWGradeTotal / HWGradeCount) * 0.2 +
          FinalProject * 0.3) *
          100,
      ) / 100;
  }
